...
contract Liquidation is OwnableUpgradeable {
        ...
        initializer
    {
        ...
        require(address(_oracleRouter) != address(0), "Liquidation: zero address");
        require(address(_rewardLocker) != address(0), "Liquidation: zero address");
    ...
    function _liquidatePosition(LiquidatePositionParams memory params, uint256[] calldata rewardEntryIds) private {
        ...
        if (params.lusdToBurn == 0) {
            ...
            require(params.lusdToBurn <= maxLusdToBurn, "Liquidation: burn amount too large");
        }
...

        {
            ...
            require(totalCollateralToMove <= totalCollateralAmount, "Liquidation: insufficient collateral"); // Insurance fund needed to resolve this
        }
    ...
    {
        amountFromStaked = MathUpgradeable.min(params.collateralWithdrawalAmount, params.stakedCollateral);
        ...
        if (amountFromLocked > 0) {
            rewardLocker.moveReward(params.user, params.liquidator, amountFromLocked, rewardEntryIds);
        }
    }
        ...
        returns (uint256 amountFromStaked, uint256 amountFromLocked)
    {
        ...
        amountFromStaked = MathUpgradeable.min(totalReward, params.stakedCollateral);
